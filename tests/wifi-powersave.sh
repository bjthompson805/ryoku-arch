#!/usr/bin/env bash
# fixture test for ryoku-wifi-powersave: Game Mode's privileged WiFi helper.
# iw is stubbed on PATH and the sysfs scan + state file point at a tmp dir =
# no real radio touched, no root needed. covers device discovery; that `off`
# saves each device's prior power-save and disables it; that `on` restores
# (defaulting to enabled when nothing was saved); and that `status` reports
# the live state.
set -euo pipefail
...
# --- off: saves prior state, then disables power-save on every device --------
"$helper" off
...
echo off >"$iwps/wlan1"           # wlan1 was already off
"$helper" off
