...
const { glyphFor, labelFor, unitFor, tempSymbol, formatTemp, parseForecast, parseLoc, parseGeo, parseJson, hhmmToSec, sunFrac } = require("./weather.js");
...
eq(parseGeo({ results: [{ name: "Pittsfield", latitude: 42.45, longitude: -73.25 }] }), { city: "Pittsfield", lat: 42.45, lon: -73.25 }, "parseGeo reads the first result");
eq(parseGeo({ results: [] }), null, "parseGeo null on an empty result list");
eq(parseGeo({ results: [{ name: "X" }] }), null, "parseGeo null when lat/lon are missing");
eq(parseGeo(null), null, "parseGeo null on null");
eq(parseGeo({}), null, "parseGeo null when results is missing");
