...
func TestVMDefaults(t *testing.T) {
	...
	v := loadVM()
	if v.Name != "ryoku-vm" || v.Guest != "linux" || v.Display != "windowed" {
...
func TestVMRoundTrip(t *testing.T) {
	...
	in := VM{
		...
		Display: "passthrough", GpuSlot: "0000:01:00.0",
	}
	if err := saveVM(in); err != nil {
		t.Fatal(err)
	}
	got := loadVM()
	if got.Cores != 8 || got.RamMB != 16384 || got.IsoPath != "/isos/win.iso" || got.GpuSlot != "0000:01:00.0" || got.Display != "passthrough" {
