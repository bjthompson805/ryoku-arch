...
Item {
    ...
    // paperLift + lineStrong, no shadow). Reused for the image-border image, a
    // rice's wallpaper, and (folders-only) a rice export target.
    component PickFile: Item {
        id: fp
        property bool active: false
        property bool foldersOnly: false
        property string title: "Choose a file"
        property url startFolder: "file://" + pg.home + "/Pictures"
        property url currentFolder: fp.startFolder
        signal picked(string path)
        signal canceled()

        function open() { fp.currentFolder = fp.startFolder; fp.active = true; }
        function goHome(sub) { fp.currentFolder = "file://" + pg.home + (sub.length ? "/" + sub : ""); }
        ...
        Rectangle {
            ...
            Text {
                ...
                text: ("" + fp.currentFolder).replace("file://", "").replace(pg.home, "~")
                color: Tokens.inkFaint
                font.family: Tokens.mono
                font.pixelSize: Tokens.fTiny
            }
