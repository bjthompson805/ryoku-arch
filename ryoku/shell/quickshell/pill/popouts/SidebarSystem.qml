...
Item {
    ...
    // thin fade-in thumb for a content pane's Flickable, matching the rest of
    // the shell's scrollbar look.
    component VScrollBar: QQC.ScrollBar {
        id: sb
        policy: QQC.ScrollBar.AsNeeded
        // a custom contentItem opts out of the style's own AsNeeded hiding,
        // so hide by hand once the content actually fits.
        visible: size < 1
        width: 5 * root.s
    ...
    Item {
        ...
        Flickable {
            ...
            boundsBehavior: Flickable.StopAtBounds
            contentWidth: width
            contentHeight: cal.implicitHeight
            QQC.ScrollBar.vertical: VScrollBar {}

            // the calendar sizes itself to its live content (month grid +
            // weather footer + the selected day's event list) instead of
            // being squashed to the pane's fixed height, so a busy day's
            // events scroll here rather than clipping inside the surface.
            Calendar {
                id: cal
                anchors.top: parent.top
                anchors.left: parent.left
                width: calFlick.width - 10 * root.s
        ...
        Item {
            ...
            Flickable {
                ...
                Column {
                    id: mediaCol
                    width: mediaFlick.width - 10 * root.s
                    spacing: 11 * root.s
        ...
        Item {
            ...
            Flickable {
                ...
                Column {
                    id: weatherCol
                    width: weatherFlick.width - 10 * root.s
                    spacing: 16 * root.s
        ...
        Item {
            ...
            Flickable {
                ...
                Column {
                    id: recordCol
                    width: recordFlick.width - 10 * root.s
                    spacing: 12 * root.s
